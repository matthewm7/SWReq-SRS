...
$<$ \begin{itemize}
    \item Shopping list creation - the shopping list creation tools will allow customers to build/fill their shopping carts with all of the items the desire to purchase and store the information for later purchase through the webpage or to take the shopping cart/list with them as they browse the store.
    \item Online checkout - through the webpage will offer the ability to customers to purchase all or some of the items in their cart in addition to selecting the type of delivery/pickup they desire.
...
$<$ There are several constraints to the system, none of these constraints focuses around integration/s of other systems or regulatory policies so far as the development team knows at this time. Reliability requirements could be a major concern for the webpage during periods of high traffic to the site and during times of low availability for certain products. Additional logging and/or auditing capabilities may be required for legitimizing transactions on the site, but these requirements have not yet been mentioned by the customer for the either the employee/shopping portions of the site. $>$
...
\end{figure}
When the user is logged in, the user will then be presented with the payment screen.  If the user does not have any stored page information or the user is a guest, they will be presented with a user interface similar to Figure \ref{payment}.  

\begin{figure}[H]
	\includegraphics[width=6 in]{UserDiagram5.png}
...
%	\clearpage
	...
	\subsubsection{Associated functional requirements}
	\paragraph[]{Delivery option – Delivery address validation}
	\begin{funreq}
...
%	\begin{flushleft}\bfseries{ID and Name: FR\thefunreq ~\hspace{.6cm} }\end{flushleft}
    ...
    The website’s storefront shall be user friendly (easy to use) to promote consumer’s usabilities.
