...
\section{Purpose}
%Need to reword, a little bit confusion
...
\section{Scope}
The Whole Knockoff Grocery Store (WKGS) program is intended to be a system to allow the Whole Knockoff grocery store to manage employees, track inventory, sell groceries online (using both delivery and in-store pickup), and give discounts to loyal customers.  The WKGS website will allow customers to quickly see what is available to purchase at the grocery store using the website storefront.  The customer can select items that they wish to purchase for delivery or in-store pickup.  After adding groceries to their virtual shopping cart, the customer can select if they would like the groceries delivered or available for in-store pickup.  The customer can then purchase these groceries through the online portal.  The website will also act as an employee hub where employees can be assigned work schedules and hours worked can be tracked.  The website also update items quanities counts to the WKGS's separate inventory system.
...
%Need to reword, I don't understand the first part of this
Customers are will be met with an item selection interface from the web page. Coupling a simple item selection interface with shopping cart and wish list capabilities our customer's customers will find the process from shopping list to checkout convenient and simplistic. All items, whether purchased through the web protal or in-store, will be communicated to the WKGS's inventory system.
...
%$<$ 
Several future dependencies could change the requirements for the \textit{WKGS} purchasing/employee website. Integration with other shopping applications could alter the amount of information displayed by the website and how the website handles user paths during their shopping experience. There are few user dependencies for the system given the online/in-person shopping alternatives that \textit{WKGS} will offer to their customers. User login and checkout will not necessary store user information and will not mandate account creation to checkout using the online system. %$>$
%Who are user? If can't use different term, need to add user into Definition
...
When the customer clicks on the cart icon, the customer can then view a list, the quantity, and the total price for the items that the customer selected.  Using an interface similar to Figure \ref{cart}, the customer can change the quantity and review their order.  If the customer is not logged in to the system, they will be prompted to login to the page shown in Figure \ref{login}.  
\begin{figure}[H]
	\mbox{
		\begin{minipage}{0.45\textwidth}
...
\end{figure}
When the customer is logged in, the customer will then be presented with the payment screen.  If the customer does not have any stored page information or the customer is a guest, they will be presented with a customer interface similar to Figure \ref{payment}.  After the customer's payment method is approved, The customer will be able to move to select how they would like to obtain their groceries.  If the customer wishes to have their groceries delivered, they will be presented with an interface similar to Figure \ref{delivery}.
...
If the customer selects pickup to obtain their groceries, they will see an interface similar to Figure \ref{pickup}.  After the customer has finished entering the required information, The customer will be presented with an order summary that shows them a confirmation of their order.  This interface is shown in Figure \ref{completed}.
...
%The application will have to interface with several hardware components for both employee and customer needs. The application will need to provide, potentially, scanning/item additions for customers and will certainly need to allow employees to scan products for both checkout and inventory stocking. Employees will be given scanners for item inventory that will look for information from the website for inventory counts and relay this information for stocking, this will need to be relayed to the online checkout portions of the website so that customers have access to the most up-to-date item inventory status.
...
The website must communicate with the inventory system for tracking and maintaining accurate inventory counts.  Using the existing customer's inventory API, the web site will communicate with the inventory system to obtain item quantities and item prices.  The web site will also deduct item quantities once purchases are complete. 
%The web page must communicate with GPS applications for delivery locations in addition to other customer devices for pickup/delivery purposes. Potentially the web page must also be able to interface with third party application which we will discuss further in Section 2 above. All/most employee activities will be performed with technology that communicates with the web page for timekeeping, customer checkout, item inventory, delivery, and other various activities in the store; all activities/technologies will run potentially independent software that must interface with website.


% \subsection{Communications Interfaces}
% The website application must have access communication to a separate inventory system from the Whole Knockoffs Grocery for inventory data; up to date inventory data is dependent on the store's inventory system. 
...
%	\subsection{Online checkout}
	...
	The checkout feature allows users to pay for their item(s) currently in the Shopping list to complete their order. Depending on type of customers, the feature should preload required customer’s information for the checkout process including: home address for delivery, billing address, and payment method. Furthermore, the customers can specify payment and delivery options on the checkout page. 
...
%	%\paragraph[]{\normalfont A user must be logged in as an existing user/member or guest.}
	...
	\begin{funreq}
		...
		TITLE: Allow everyone to checkout online\\
		...
		%\begin{table}[h!]
			\resizebox{\textwidth}{!}{%
				\begin{tabularx}{\columnwidth}{|l|X|}
					...
					The checkout feature should let guest customer complete online order with the required information, including payment and home delivery/in-store pickup options             \\ \hline
					Trigger:         & Guest customer clicks on option “checkout”                    \\ \hline
					Preconditions    & None                                       \\ \hline
					Postconditions   & Guest customer successfully complete checkout for online orders, Payment received\\ \hline
					Normal Flow &
					\bfseries{Allow everyone to checkout online – Guest Customers}\normalfont\newline 
		...
		%\begin{table}[h!]
			\resizebox{\textwidth}{!}{%
				\begin{tabularx}{\columnwidth}{|l|X|}
					...
					The checkout feature should let member customer complete online order with the required information, including payment and home delivery/in-store pickup options
					\\ \hline
					Trigger:         & Member customer clicks on option “checkout”                    \\ \hline
					Preconditions    & Logged in as a member                                       \\ \hline
					Postconditions   & Member customer successfully complete checkout for online orders, Payment received\\ \hline
					Normal Flow &
					\bfseries{Allow everyone to checkout online – Member Customer}\normalfont\newline 
					\begin{enumerate}
						\item Member customer selects checkout option
					
						\item Website checks for type of customer
					
						\item Website established member customer
					
						\item Website preloads member information: name and address
					
						\item Website validates accurate pricing with additional taxes for total price
					
						\item Website requests member customer the following information: payment information, billing address, option for home deliver or in-store pickup \textit{Note: should website keep payment information for customer}s?
...
%\improvement[inline]{\newline This section is incomplete}
\subsubsection{Introduction/Purpose of feature}
This feature allows customers to create and edit settings for grocery store accounts.  New customer will use this feature to populate basic account information while setting up their account such as username and password.  In addition, this feature allows existing customer (member) a convenient way to edit their password, change delivery addresses, manage stored payment information, and change other stored personal information.  From the customer management profile, customers will also be able to see past purchases and orders.
...
\begin{funreq}
	...
	RAT: In order for a customer to register an account\\
	DEP: None\\
	...
	\begin{table}[H]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabularx}{\columnwidth}{|l|X|}
				\hline
				Description: &
				A customer should be able to register through the website. The customer must provide a name, username, password and e-mail address. \\ \hline
				Trigger:         & Customer clicks on "Register" option                    \\ \hline
				Preconditions    & None                                       \\ \hline
				Postconditions   & Account is created, Customer is Logged in      \\ \hline
				Normal Flow &
				\bfseries{Create a new Member account}\normalfont\newline 
				\begin{enumerate}
					\item Customer Clicks on Register Enters name, user-name, password and e-mail address.
				%Needs clarify****************************************
				\item Website confirms it is a unique entry and save the new member details
				\item Website Informs customer of successful creation
				\item Website Logs customer in 
				\end{enumerate}
				\\ \hline
				Alternative Flow & None                                       \\ \hline
	...
	\begin{table}[H]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabularx}{\columnwidth}{|l|X|}
				\hline
				Description: &
				A user should be able to register through the website. The user must provide a name, username, password and e-mail address. \\ \hline
				Trigger:         & Customer selects category to filter by         \\ \hline
				...
				Normal Flow &
				\begin{enumerate}
				\item The customer selects a store category from the homepage of the website
				\item The website shall update the list of displayed items with the selected category
				\end{enumerate}
				\\ \hline
				Alternative Flow & None                                       \\ \hline
...
\begin{funreq}
	\label{store_search}
	TITLE: Search for available store items\\
...
%\paragraph[]{\normalfont The search option must return storefront items that match the customer’s query.}
	...
	%\improvement[inline]{\newline This section is incomplete}
	\subsubsection{Introduction/Purpose of feature}
	The shopping cart/list feature allows the user to manage their selected store item(s). The feature includes information on item's price, availability, and location in the store. In addition, the user can change the quantity of item(s) to be purchased or remove item(s) from the shopping cart/list.
...
%****************************************************************************************************************
%Not sure what is this requirement doing, ALL customers can add items into cart. Add item into shopping cart is simple and I don't think it reuqires this table format
%****************************************************************************************************************	
%%\paragraph[]{Functional requirement %\arabic{subsubsection}.\arabic{paragraph}}
%\paragraph[]{\Subsectionname ~functional requirement \arabic{paragraph}}
%\begin{funreq}
...
%	\end{funreq}
%*****************************************************************************************************************	
%I think we have to do rework shopping cart/list
%****************************************************
...
\end{funreq}
	...
	\begin{funreq}
		...
		DESC: Managers shall be able to schedule work days to employees.  The manager must be able to set which hours for every day of the week for each employee.\\
		...
		\begin{table}[H]
			...
			\resizebox{\textwidth}{!}{%
				\begin{tabularx}{\columnwidth}{|l|X|}
					\hline Description: &
					    Managers shall be able to schedule work days to employees.  The manager must be able to set which hours for every day of the week for each employee.
					\\ \hline Trigger: & 
					Manager clicks on an employee to assign hours in the work scheduler portion of the website                 
					\\ \hline Preconditions & 
					The employee exists and is able to be assigned work hours
					\\ \hline Postconditions   & 
					Employee has hours assigned for the next work week stored for their account
					\\ \hline Normal Flow &
					\bfseries{Manager assigns work hours}\normalfont\newline 
					\begin{enumerate}
    					\item Manager shall select a user from a dropdown box
    					\item Manager shall select a week from a dropdown box
	...
	\begin{funreq}
		...
		TITLE: Employee view assigned work shedule\\
...
%	\begin{flushleft}\bfseries{ID and Name: FR\thefunreq ~\hspace{.6cm} }\end{flushleft}
		...
		\begin{table}[H]
			...
			\begin{tabularx}{\columnwidth}{|X|X|X|X|}
				\hline
				Created By    & Thoai Mai & Date Created    & 03/23/2020 
				\\ \hline
				Primary Actor & Customer & Secondary Actor & Website 
				\\ \hline
			\end{tabularx}
			\resizebox{\textwidth}{!}{
				\begin{tabularx}{\columnwidth}{|l|X|}
					\hline Description: &
				    The website shall check for customer’s choice of delivery or in-store pickup for payment processed online order and validate for delivery address.
					\\ \hline Trigger: & 
					Customer clicks on option “Place Order”               
					\\ \hline Preconditions & 
					...
					\begin{enumerate}
					    ...
					    \item If delivery address is missing, the website shall prompt customer for delivery address and option to have online order as in-store pickup
		...
		\begin{table}[H]
			...
			\resizebox{\textwidth}{!}{%
				\begin{tabularx}{\columnwidth}{|l|X|}
					...
					Customer clicks on option “Submit” in \reqref{grocery_deliveryaddr}
					\\ \hline Preconditions & 
					Payment was successfully received
					
					Website obtains all required customer’s information
...
%	\begin{flushleft}\bfseries{ID and Name: FR\thefunreq ~\hspace{.6cm} }\end{flushleft}
		...
		\begin{table}[H]
			...
			\resizebox{\textwidth}{!}{%
				\begin{tabularx}{\columnwidth}{|l|X|}
					\hline Description: &
				    The website shall check for customer’s choice of in-store pickup for payment processed online order and validate for pickup time.
					\\ \hline Trigger: & 
					Customer clicks on option “Place Order”
					\\ \hline Preconditions & 
					...
					Website obtains all required customer’s information
					\\ \hline Postconditions   & 
					Website obtains customer’s desired in-store pickup time
...
%	\begin{flushleft}\bfseries{ID and Name: FR\thefunreq ~\hspace{.6cm} }\end{flushleft}
		...
		\begin{table}[H]
			...
			\resizebox{\textwidth}{!}{%
				\begin{tabularx}{\columnwidth}{|l|X|}
				    ...
				    The website shall check for customer’s choice of in-store pickup for payment processed online order and validate for pickup time.
					\\ \hline Trigger: & 
					Customer clicks on option “Submit” in \reqref{grocery_pickuptime}
					\\ \hline Preconditions & 
					Payment was successfully received
    ...
    The website shall protect consumer’s personal data.
