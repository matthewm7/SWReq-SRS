 S%Copyright 2014 Jean-Philippe Eisenbarth
...
This document provides detailed requirements about the product, Whole Knockoffs webpage, that our team is creating for our customer, the owners of Whole Knockoffs.  This document is intended %for said customers and our software team
...
%Customers will be met with an item selection interface from the web page. Coupling a simple item selection interface with shopping cart and wish list capabilities our customer's customers will find the process from shopping list to checkout convenient and simplistic. All items, whether purchased through the web portal or in-store, will be communicated to the WKGS's inventory system.
	...
	\begin{table}[H]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabularx}{\columnwidth}{|l|X|}
				...
				Preconditions    & None                                       \\ \hline
				Postconditions   & Account is created, member is Logged in      \\ \hline
				Normal Flow &
				\bfseries{Create a new Member account}\normalfont\newline 
				\begin{enumerate}
				\item Customer Clicks on "Register".
			 	\item Customer enters name, user-name, password, and e-mail address.
				%Needs clarify****************************************
				\item Website confirms it is a unique entry and saves the new member details.
				\item Website informs customer of successful creation.
				\item Website logs the member in.
				\end{enumerate}
				\\ \hline
				Alternative Flow & None                                       \\ \hline
...
\begin{funreq}
	...
	RAT: In order for an employee to register an account\\
	DEP: None\\
\end{funreq}
	...
	\begin{table}[H]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabularx}{\columnwidth}{|l|X|}
				\hline
				Description: &
				A customer should be able to select a category on the website to filter the item type displayed. \\ \hline
				Trigger:         & A customer selects a category filter         \\ \hline
				Preconditions    & Items have a category specified             \\ \hline
				Postconditions   & The storefront displays only items that have the specified category    \\ \hline
				Normal Flow &
				\begin{enumerate}
				...
				Alternative Flow & 
				\begin{enumerate}
				\item The customer selects an item from the homepage
				\end{enumerate}
				\\ \hline
...
%	The customer can edit item’s quantity and remove item from shopping list/cart. When the users are satisfied with their selected items, they can click the “Checkout” option to begin the checkout process. 
%	
%	Existing members:
...
\end{funreq}
	...
	\begin{funreq}
		...
		\begin{table}[H]
			\begin{tabularx}{\columnwidth}{|X|X|X|X|}
				\hline
				Created By    & Matthew Moore & Date Created    & 03/123/2020 \\ \hline
